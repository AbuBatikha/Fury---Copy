...
Boolean programOn = false, pause = false, musicPaused = false, soundPlayed = false, sheildApp = false, mouseOverLogo, laserPlayed = false, cleanAnimation= false, Idle = true, menuOn = true, menuChoicesOn, combat = false, laser = false, skyCleared = false, MegaLaserReady = false, drainingMegaLaser = false, sayCapacity = false, dontSayAgain = false;
...
void settings() {
  size(295, 350);
  img = loadImage("background_fury.jpg");
  sheildLogo = loadImage("images.png");
  sheildLogo.resize(47, 20);
}
...
void draw() {
  ...
  if (programOn == false) {
    ...
    noStroke();
    textSize(20);
    text("Press Enter to", 87, 105);
    text("continue", 112, 130);
    ...
    if (laser == true) {
      ...
      line(2*width/4, height+100, mouseX+5, mouseY);
...
void mouseMoved() {
  ...
  if ((mouseX >= 16 && mouseX <= 66 && mouseY >= 306 && mouseY <=323)||(mouseX >= 9 && mouseX <= 93 && mouseY <= 204 && mouseY >= 175)||(mouseX >= 9 && mouseX <= 93 && mouseY <= 153 && mouseY >= 127)||(mouseX >= 9 && mouseX <= 93 && mouseY <= 107 && mouseY >= 78)||(mouseX >= 5 && mouseX <= 64 && mouseY >= 4 && mouseY <= 64)) {
  } else {
    soundPlayed = false;
  }
}
...
public void clearSoundEffects() {
  ...
  if (CombatSystemOff.isPlaying() == true) {
    CombatSystemOff.stop();
  }
